...
PII_PATTERNS = [
    ...
    r"\b\d{6,12}\b",  # MRN (generic) #Medical record number? generic 6-12 lengths of pure digits
]
...
INDIVIDUAL_MARKER_FIELDS = {
    "nama", "nama_lengkap", "fullname", "full_name", "marga",
    "nama_depan", "nama_belakang", "first_name", "last_name",
    "inisial", "initial", "nama_ibu", "nama ibu", "nama_ayah", "nama ayah",
    "nama_pasangan", "nama pasangan",
    "nama_wali", "nama wali", "wali",
    "kontak_darurat", "kontak darurat", "emergency_contact",
    "gelar", "title",
    "tanggal_lahir", "tgl_lahir", "tgl lahir", "tanggallahir", "dob", "birth_date",
    "tempat_lahir", "tmp_lahir", "tmp lahir", "birth_place",
    "nik", "no_ktp", "nomor_ktp", "nomor ktp", "ktp",
    "kk", "no_kk", "nomor_kk", "nomor kk",
    "paspor", "passport", "no_paspor", "nomor_paspor",
    "npwp", "no_npwp", "nomor_npwp",
    "bpjs", "no_bpjs", "nomor_bpjs",
    "sim", "no_sim", "nomor_sim",
    "alamat", "alamat_lengkap", "alamat lengkap",
    "domisili", "tempat_tinggal", "tempat tinggal", "tinggal",
    "alamat_domisili", "alamat domisili", "rumah", "home",
    "alamat_ktp", "alamat ktp", "gps", "koordinat", "coordinate", "coordinates",
    "latitude", "lat", "longitude", "lon", "lng",
    "lokasi", "location",
    "telepon", "no_telepon", "nomor_telepon", "nomor telepon",
    "hp", "no_hp", "nomor_hp", "handphone", "ponsel", "mobile", "telfon", "telphone", "phone",
    "whatsapp", "wa", "email", "e-mail",
    "mrn", "medical_record_number",
    "no_rm", "nomor_rm", "nomor rm", "rekam_medis", "rekam medis",
    "no_mr", "nomor_mr",
...
METADATA_KEY_PII_PATTERNS = [
    ...
    r"(?i)\b(?:mrn|medical[_ -]?record[_ -]?number|no[_ -]?rm|nomor[_ -]?rm|no[_ -]?mr|nomor[_ -]?mr|rekam[_ -]?medis)\b",
]
    ...
    with open(input_path, "r", newline="", encoding="utf-8") as infile:
        ...
        with open(output_path, "w", newline="", encoding="utf-8") as outfile:
            ...
            for row in reader:
                ...
                filtered_row = [
                    anonymise(val) if (is_individual and NAME_PATTERN.search(header[idx])) else val
                    for idx, val in enumerate(row) if idx in columns_to_keep
                ]
            ...
            if event == "start_map":
                ...
                if "name_strings" in stack[-1]:
                    if stack[-1].get("is_individual"):
                        ...
                        names = stack[-1].pop("name_strings")
                    if not stack[-1]["first"]:
                        outfile.write(",")
                    outfile.write(",".join(names))
                stack[-1].pop("inside_name", None)  # reset flag
                outfile.write("}")
                stack.pop()
                ...
                if NAME_PATTERN.search(value):
                    ...
                    stack[-1]["inside_name"] = True          # flag
                    outfile = io.StringIO()                  # buffer isi “name”
                ...
                if stack:
                    if stack[-1]["type"] == "object" and stack[-1].get("pending_key"):
                        ...
                        if not stack[-1]["first"]:
                            outfile.write(",")
                if event == "string":
                    if stack[-1].get("inside_name"):
                        outfile.write(json.dumps(value))          # tulis original value dari nama
                        stack[-1].pop("inside_name", None)        # ← tambahkan baris ini
                    else:
