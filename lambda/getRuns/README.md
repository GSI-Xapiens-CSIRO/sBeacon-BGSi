...
## Overview
Handles queries for sequencing run records in the sBeacon API.

## Routes

| Path | Method | Middleware |
|------|--------|------------|
| `/runs` | POST | Permission + Quota |
| `/runs/filtering_terms` | POST | Permission + Quota |
| `/runs/{id}` | POST | Permission + Quota |
| `/runs/{id}/g_variants` | POST | Permission + Quota |
| `/runs/{id}/analyses` | POST | Permission + Quota |
